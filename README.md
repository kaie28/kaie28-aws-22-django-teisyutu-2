...
・今回は私が好きな「Instagram」のアプリをヒントに、プログラミング言語やAWSを選定し、全ての工程を独学で行いました。プログラミングは未経験ですが、3か月で完了できるように、勉強内容の順序や学習範囲など入念に計画し、特に「セキュリティ強化」「データ復元の簡易化」「コスト面」を重視して、工夫しながら構築・設計しました。
...
★【目標1】サーバーやアプリでの修正の簡易さ、自動化の導入を推進する。

・継承を推進する(修正がしやすくなる)。

・自動化を導入する(人の代わりに24時間働く)。


★【目標2】本質の仕様そのものが変化した場合、簡易的に変更できる状態にする。
